...
使用 npm 全局安装或 GitHub Release `.tgz` 全局安装的实例，可以在“设置 → 更新”中完成以下操作：

1. “检测更新”访问 Questra 的 GitHub Releases，比较当前版本与最新正式 Release。检测不会自动安装内容，也不会采用草稿或预发布版本；当前版本高于最新正式版本时，页面以黄色警示显示“开发预览版”。页面中的 Release 链接统一指向 Releases 列表。
...
更新页面始终安装 npm 全局包，不会修改 Git 源码目录。通过源码运行时，请先备份数据，再在仓库中获取目标版本、重新安装依赖和构建前端，最后重启源码进程。若当前全局命令由 `npm link` 指向源码，后台安装也不能替代源码的版本控制与构建流程。
