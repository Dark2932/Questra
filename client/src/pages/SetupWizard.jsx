...
export default function SetupWizard({ onComplete }) {
  ...
  return <div className="setup-shell">
    <Card className="setup-card" bordered={false}>
      ...
      <Steps current={current} items={[{ title: '欢迎' }, { title: '管理员账户' }, { title: '站点设置' }]} />
      {error && <Alert type="error" showIcon message={error} style={{ marginTop: 24 }} />}
      {current === 0 && <Result icon={<CheckCircleOutlined />} title="欢迎使用 Questra" subTitle="只需几步即可完成站点初始化。你的管理员账户只有一组，密码会以安全哈希保存。" extra={<Button type="primary" icon={<ArrowRightOutlined />} onClick={() => setCurrent(1)}>开始设置</Button>} />}
