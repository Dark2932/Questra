...
export default function SurveyDialog({ open, onClose, questions, groups = [], editing, onSubmit }) {
  ...
  const submit = async () => {
    try {
      ...
      const payload = { kind, title: values.title.trim(), description: (values.description || '').trim(), status: expired ? 'closed' : (values.status || 'active'), expiresAt: values.expiresAt ? values.expiresAt.toISOString() : null, selectionMode: mode };
