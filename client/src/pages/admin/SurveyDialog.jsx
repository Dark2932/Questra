...
export default function SurveyDialog({ open, onClose, questions, groups = [], editing, onSubmit }) {
  ...
  useEffect(() => { if (!open) return; form.resetFields(); setKind(editing?.kind || 'survey'); setMode(editing?.selectionMode || 'manual'); setSelected(editing?.questions?.map((q) => q.poolQuestionId).filter(Boolean) || []); setSourceGroup(editing?.sourceGroupId ?? 'all'); setManualGroup('all'); setScoringMode(editing?.scoringMode || 'weighted'); form.setFieldsValue({ title: editing?.title || '', description: editing?.description || '', kind: editing?.kind || 'survey', selectionMode: editing?.selectionMode || 'manual', sourceGroupId: editing?.sourceGroupId ?? 'all', randomCounts: editing?.selectionConfig?.randomCounts || {}, scoringMode: editing?.scoringMode || 'weighted', totalScore: editing?.maxScore || 100, typeWeights: editing?.scoringConfig?.typeWeights || {}, questionScores: editing?.scoringConfig?.questionScores || {} }); }, [open, editing, form]);
